...
const actionLabels: Record<string, { label: string; color: string }> = {
  added: { label: 'Hinzugefügt', color: tokens.success },
  edited: { label: 'Bearbeitet', color: '#f59e0b' },
  deleted: { label: 'Gelöscht', color: tokens.danger },
};
...
export function ChangelogList({ entries, onUndo }: ChangelogListProps) {
  if (entries.length === 0) {
    return (
      <div style={{ textAlign: 'center', padding: '40px 20px', color: tokens.muted }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      {entries.map((entry) => {
        ...
        const info = actionLabels[entry.action] ?? { label: entry.action, color: tokens.muted };
        ...
        return (
          ...
          >
            ...
            <div style={{ minWidth: 0, flex: 1 }}>
              ...
              <div style={{ fontSize: 11, color: tokens.muted, display: 'flex', gap: 8 }}>
                <span>{getGroupName(entry.groupId)}</span>
                <span>·</span>
                <span style={{ fontFamily: tokens.font.mono }}>{date} {time}</span>
              </div>
            </div>
